 ...
 */
public class DurationSimpleTest extends BuiltInSymmetryTest {
    ...
    public void assertWrite(final Jsonb jsonb) {
        ...
        widget.setValue(Duration.parse("PT1H30M"));
        assertEquals("{\"value\":\"PT1H30M\"}", jsonb.toJson(widget));
    }

    @Override
    public void assertRead(final Jsonb jsonb) {
        final Widget widget = jsonb.fromJson("{\"value\":\"PT1H30M\"}", Widget.class);
        assertEquals(Duration.parse("PT1H30M"), widget.getValue());
    }
}
